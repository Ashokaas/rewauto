...
driver = webdriver.Chrome(executable_path=r'driver\chromedriver110.exe')
    ...
    #            ____________________________________
        ...
        if check_element('By.ID', 'passwordError') == True:
            if driver.find_element(By.ID, 'passwordError').text in 'Votre compte ou mot de passe est incorrect. Si vous avez oublié votre mot de passe, redéfinissez-le ici.':
                driver.close()
                playsound(sound='sound_effects/error.mp3')
        ...
        'link' : driver.find_element(By.XPATH, f'//*[@id="more-activities"]/div/mee-card[{p+1}]')
                ...
                print(f"L'élément {p+1} n'est pas un lien qui rapporte des points")
