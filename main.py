...
def recherches(nb_pc:int=40, nb_tel:int=30, liste_recherches:list=['banane', 'oui']):
    ...
    for pc in range(nb_pc):
        raccourci(['ctrl', 'l'])
        pyautogui.press('backspace')
        pyautogui.write(f'bing.com/search?q={liste_recherches[randint(0, len(liste_recherches)-1)]}')
        pyautogui.press('enter')
    ...
    for truc in set:
        ...
        if truc['check'] == False:
            if truc['type'] == 'normal_link':
                ...
                sleep(1)
                for nb in range(3):
                    for nb_res in range(8):
                        pyautogui.click(221+128*nb_res, 926)
                        pause_pour_veski_le_bot(tps_en_moins=2)
                    pause_pour_veski_le_bot()
                raccourci(['ctrl', 'w'])
